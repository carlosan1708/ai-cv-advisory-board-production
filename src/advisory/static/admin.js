(() => {
  ...
  document.querySelector("[data-invite-form]").addEventListener("submit", async (event) => { event.preventDefault(); const error = document.querySelector("[data-invite-error]"); error.textContent = ""; try { await api("/api/admin/access", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(event.currentTarget))) }); event.currentTarget.reset(); await load(); } catch (exc) { error.textContent = exc.message; } });
  ...
  else window.addEventListener("load", () => { const error = gate.querySelector("[data-auth-error]"); if (!body.dataset.clientId || !window.google?.accounts?.id) { error.textContent = "Google sign-in could not be loaded."; return; } google.accounts.id.initialize({ client_id: body.dataset.clientId, callback: (response) => { token = response.credential; bootstrap().catch((exc) => { error.textContent = exc.message; }); } }); google.accounts.id.renderButton(gate.querySelector("[data-google-signin]"), { theme: "outline", size: "large" }); });
})();
